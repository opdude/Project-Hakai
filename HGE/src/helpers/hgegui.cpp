...
hgeGUI::hgeGUI(float screenX, float screenY)
{
...
int hgeGUI::Update(float gt, float dt, bool checkInput)
{
	bool bDone;
	int key;
	hgeGUIObject *ctrl;
...
// Update the mouse variables
	...
	mx = mx * screenX / hge->System_GetState(HGE_SCREENWIDTH);
	my = my * screenY / hge->System_GetState(HGE_SCREENHEIGHT);
