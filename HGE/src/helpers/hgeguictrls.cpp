...
hgeGUIListbox::hgeGUIListbox(int _id, float x, float y, float w, float h, hgeFont *fnt, DWORD tColor, DWORD thColor, DWORD hColor)
{
	...
	rect.Set(x, y, x+w, y+h);
	font=fnt;
	sprHighlight=new hgeSprite(0, 0, 0, w, fnt->GetScaledHeight());
...
void hgeGUIListbox::Render()
{
	...
	for(i=0;i<GetNumRows();i++)
	{
		...
		{
			sprHighlight->Render(rect.x1,rect.y1+i*font->GetScaledHeight());
			font->SetColor(texthilColor);
		}
		else
			font->SetColor(textColor);

		font->Render(rect.x1+3, rect.y1+i*font->GetScaledHeight(), HGETEXT_LEFT, pItem->text);
		pItem=pItem->next;
	}
}
...
bool hgeGUIListbox::MouseLButton(bool bDown)
{
	...
	if(bDown)
	{
		nItem=nTopItem+int(my)/int(font->GetScaledHeight());
		if(nItem<nItems)
		{
			nSelectedItem=nItem;
			return true;
